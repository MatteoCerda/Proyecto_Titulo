...
export class AdminUsuariosRolesPage {
  ...
  allSelected() { const list = this.filteredSorted(); return list.length>0 && list.every(u=>!!u.selected); }
  toggleSelectAll(ev: any) { const ck = !!ev?.target?.checked; this.filteredSorted().forEach(u => u.selected = ck); }
  hasSelection() { return this.users().some(u => u.selected); }

  roleLabel(r: Role) { return r==='ADMIN' ? 'Administrador' : r==='CLIENT' ? 'Cliente' : 'Operador'; }
  ...
  remove() {
    ...
    if (ids.length===0) return; if (!confirm('¿Eliminar usuarios seleccionados?')) return;
